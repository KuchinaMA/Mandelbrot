...
Результаты измерений в таблице ниже и далее представлены в количестве тиков процессора (нужно умножить на $10^6$).

| Уровень оптимизации | $t_1$ | $t_2$   | $t_3$ | $t_{ср} $|
|---------------------|-----------|-----------|-----------|-----------------|
| -O0                 | 894.4      | 884.5      | 904.5      | 894.4  ± 10.0     |
| -O3                 | 420.5      | 425.6      | 430.0      | 425.4 ± 4.8       |

Таким образом, применение ключа оптимизации `-O3` ускоряет время работы программы в $\frac{894.4}{425.4} ≈ 2.10 ± 0.02$ раз.
...
|  $t_1$ | $t_2$   | $t_3$ | $t_{ср} $|
|---------------------|-----------|----------|-----------------|
| 595.4      | 589.5      | 589.8     | 591.6 ± 0.3   |

Относительно первой версии с уровнем оптимизации `-O3` получаем ускорение в $\frac{425.4}{591.6} ≈ 7. 19 ± 0.01$ раз. По сравнениею со 2-ой и 3-ей версиями это более значительное ускорение, однако оно всё равно не достигает 8 раз. Возможные причины:

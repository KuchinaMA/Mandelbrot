...
| -O3                 | 421     | 426     | 430     | 425 ± 5       |

Таким образом, применение ключа оптимизации `-O3` ускоряет время работы программы в $\frac{894}{425} ≈ 2.10 ± 0.02$ раз.
...
Относительно первой версии с уровнем оптимизации `-O3` получаем ускорение всего в $\frac{425}{195} ≈ 2.18 ± 0.05$ раз. Это связано с тем, что компилятор всё-таки смог обнаружить некоторые возможности для оптимизаций, но к параллельным вычислениям всё-таки не перешёл. 
...
Относительно первой версии c уровнем оптимизации `-O3` получаем ускорение в $\frac{425}{207} ≈ 2.06 ± 0.07$ раза. Заметим, что эта версия получилась медленнее второй. 
...
Относительно первой версии с уровнем оптимизации `-O3` получаем ускорение в $\frac{4250}{592} ≈ 7. 19 ± 0.01$ раз. По сравнениею со 2-ой и 3-ей версиями это более значительное ускорение, однако оно всё равно не достигает 8 раз. Возможные причины:
